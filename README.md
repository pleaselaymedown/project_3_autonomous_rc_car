# 🚗 Project_3 Smart RC Car 🚗
<br>

## 1. Project Summary (프로젝트 요약)

STM32F411RE 보드를 기반으로 제작한 스마트 RC카 프로젝트입니다.
블루투스 UART 통신을 이용해 전진, 후진, 좌회전, 우회전, 정지 명령을 수동으로 제어할 수 있으며, 전방/좌측/우측 초음파 센서를 이용해 장애물을 감지하고 자율주행 모드에서 회피 주행을 수행하도록 구현했습니다.

<br>

## ✨ 2. Key Features (주요 기능)

* 블루투스 명령을 이용한 RC카 수동 주행 제어
* `전진 / 후진 / 좌회전 / 우회전 / 정지` 동작 지원
* 블루투스 명령으로 수동주행 모드와 자율주행 모드 전환
* 전방, 좌측, 우측 3개의 초음파 센서를 이용한 거리 측정
* 장애물 감지 시 후진 후 좌우 거리 비교를 통한 회피 방향 결정
* 상태 머신 기반 자율주행 로직 구현
* USART2 `printf` 출력으로 현재 상태와 센서 거리값 디버깅 가능
* 
<br>
...
## 7. Autonomous Driving Logic (자율주행 로직)
...
## 8.1 초음파 센서 3개가 서로 간섭하여 거리값이 튀는 문제

#### 문제 상황

전방, 좌측, 우측 초음파 센서를 연속으로 사용할 때 Echo 신호가 겹치면서 거리값이 불안정하게 측정되는 문제가 발생했습니다.

#### ❓ 원인 분석

초음파 센서는 Trigger 신호를 보낸 후 Echo 신호가 돌아오는 시간을 측정하는 방식입니다. 여러 센서를 거의 동시에 동작시키면 한 센서의 초음파가 다른 센서의 Echo 측정에 영향을 줄 수 있습니다.

#### ❗ 해결 방법

각 센서를 동시에 Trigger하지 않고, 전방 → 좌측 → 우측 순서로 측정하도록 분리했습니다.
...
센서 사이에 짧은 지연을 넣어 초음파 신호 간섭을 줄였습니다.

---

## 8.2 초음파 센서값이 0cm로 튀면서 장애물 판단이 불안정한 문제
...
`3cm` 이하의 비정상적인 값은 실제 장애물로 판단하지 않고, 임시로 먼 거리값인 `999`로 처리했습니다.
